...
pub(crate) fn physical_volumes_to_deactivate<P: AsRef<Path>>(paths: &[P]) -> Vec<PathBuf> {
    ...
    device_maps(|pv| {
        info!("libdistinst: checking if {} needs to be marked", pv.display());
        if let Ok(path) = read_link(pv) {
            ...
            let slave_path = {
                ...
                slave_components.iter().for_each(|comp| slave_path.push(comp));
                slave_path
            };
            ...
            let _ = read_dirs(&slave_path, |slave| {
                ...
                if paths.iter().any(|p| p.as_ref().file_name().unwrap() == slave_path) {
