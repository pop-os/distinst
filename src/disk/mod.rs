...
impl Display for DiskError {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            MountsObtain { ref why } => writeln!(f, "unable to get moutns: {}", why)
        }
    }
}
...
impl Disk {
    pub fn new(device: &mut Device) -> Result<Disk, DiskError> {
    ...
    pub fn from_name(name: &str) -> Result<Disk, DiskError> {
...
impl PartitionInfo {
    pub fn new(partition: &Partition, is_msdos: bool) -> io::Result<Option<PartitionInfo>> {
        ...
        Ok(Some(PartitionInfo {
            ...
            name: if is_msdos { None } else { partition.name().map(String::from) },
    ...
    pub fn is_swap(&self) -> bool {
        self.filesystem.map_or(false, |fs| fs == FileSystemType::Swap)
    }
