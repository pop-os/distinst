...
pub(crate) fn detect_recovery() -> Option<RecoveryOption> {
    ...
    if recovery_path.exists() {
        ...
        return Some(RecoveryOption {
            ...
            kbd_model:     env.get("KBD_MODEL").map(String::from),
            kbd_variant:   env.get("KBD_VARIANT").map(String::from),
            efi_uuid:      env.get("EFI_UUID").map(String::from),
            ...
            luks_uuid:     env.get("LUKS_UUID").map(String::from),
            upgrade_mode:  env.get("UPGRADE").map_or(false, |oem| oem == "1"),
            refresh_mode:  env.get("REFRESH").map_or(false, |oem| oem == "1"),
        });
    }
