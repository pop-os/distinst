...
pub fn get_device<'a, P: AsRef<Path>>(name: P) -> io::Result<Device<'a>> {
    ...
    Device::get(device).map_err(|why| io::Error::new(
        why.kind(),
...
pub fn open_device<'a, P: AsRef<Path>>(name: P) -> io::Result<Device<'a>> {
    ...
    Device::new(device).map_err(|why| io::Error::new(
        why.kind(),
...
pub fn commit(disk: &mut PedDisk) -> io::Result<()> {
    ...
    disk.commit().map_err(|why| io::Error::new(
        why.kind(),
        format!(
...
pub fn sync(device: &mut Device) -> io::Result<()> {
    ...
    device.sync().map_err(|why| io::Error::new(
        why.kind(),
        format!(
            "failed to sync libparted device at {:?}: {}",
            device.path(),
            why
        )
    ))
